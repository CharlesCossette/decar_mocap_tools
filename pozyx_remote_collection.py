        ...
        for idx_pozyx, pozyx in enumerate(self.pozyxs):
            ...
            for idx, id in enumerate(device_list.data):
                if id in self.ids and not self.allow_self_ranging:
                    ...
                    if self.record_range:
                        status_range = pozyx.doRanging(id, device_range,remote_id = self.remote_id)
                    ...
                    if self.record_accel:
                        status = pozyx.getAcceleration_mg(accel_data,remote_id = self.remote_id)
                        data_string += str(accel_data.x) + "," 
                        data_string += str(accel_data.y) + "," 
                        data_string += str(accel_data.z) + ","

                    if self.record_gyro:
                        status = pozyx.getAngularVelocity_dps(gyro_data,remote_id = self.remote_id)
                        data_string += str(gyro_data.x) + "," 
                        data_string += str(gyro_data.y) + "," 
                        data_string += str(gyro_data.z) + ","

                    if self.record_mag:
                        status = pozyx.getMagnetic_uT(mag_data,remote_id = self.remote_id)
                        data_string += str(mag_data.x) + "," 
                        data_string += str(mag_data.y) + "," 
                        data_string += str(mag_data.z) + ","

                    if self.record_quat:
                        status = pozyx.getNormalizedQuaternion(quat_data,remote_id = self.remote_id)
                    ...
                    if self.record_pres:
                        status = pozyx.getPressure_Pa(pres_data,remote_id = self.remote_id)
...
if __name__ == "__main__":
    ...
    dc.record(150)      # To stream data to screen and save to a file
